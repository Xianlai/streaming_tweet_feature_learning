# Installing and running pyspark on Mac OSX

We have 2 ways to config Spark environment before starting using it. The regular way is installing Java, Scala and Spark one by one. And the other way is installing docker and pull an configured pyspark image from the internet.
...
```
The first line checks the version of docker. And the 2nd line verifies that Docker is pulling images and running as expected.
...
### Pip install pyspark package
```
$ pip install pyspark
```

# Running Spark
1. Go to where our spark installation folder is, for example: 
/usr/local/spark/spark-2.2.0-bin-hadoop2.7/

2. Run Spark either by coding in shell(for Python, we can use Python, IPython or Jupyter) interactively or directly launching applications with command `spark-submit`. Either way, we should tell the spark which master machine we want to connect to and what the configuration of Spark we would like to use.
